...
+ Currently successfully building with Bazel 6.5.0 as seen in the [Dockerfile](Dockerfile).
+ Consider installing the Bazel for Clion plugin in JetBrains Clion IDE for it to be able to fully navigate the project's source code (e.g. build-time generated pb.h source files which are expected by the C++ sources but only generated by bazel into its ~/.cache/bazel path from protobuf files, are unknown to the IDE without this plugin, and without you clicking its "Sync" button, which breaks chains of IDE source code navigation needlessly; reference: https://chatgpt.com/s/t_68d304c819c081918ff51689dc885d71)
  + It makes you able to go to definition inside the recursive graph of bazel BUILD files.
  + It makes the editor aware of pb.h files which are build by bazel from the underlying protobuf definitions, without which most code symbols are marked as unknown in the editor. 
  + The above IDE awareness is only materialized after you perform the "Sync" action in the Bazel menu or in context menus ― it basically draws the bazel build graph into the IDE's project model to make the IDE understand the project code.
