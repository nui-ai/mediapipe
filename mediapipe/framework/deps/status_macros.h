...
//   }
//
// The macro ends with a `hand_tracking_mp_lean::StatusBuilder` which allows the returned
...
//   }
//
// `hand_tracking_mp_lean::StatusBuilder` supports adapting the builder chain using a
...
// interfaces.  See `hand_tracking_mp_lean::TaskReturn` and `rpc::RpcSetStatus` for
// details.
//
//   void Read(StringPiece name, hand_tracking_mp_lean::Task* task) {
//     int64 id;
...
// to avoid confusion between a `hand_tracking_mp_lean::StatusBuilder` and a
...
#define MP_RETURN_IF_ERROR(expr)                                     \
  ...
  if (hand_tracking_mp_lean::status_macro_internal::StatusAdaptorForMacros       \
...
// well as a `hand_tracking_mp_lean::StatusBuilder` object populated with the error and
...
#define MP_STATUS_MACROS_IMPL_MP_ASSIGN_OR_RETURN_2_(lhs, rexpr)               \
  MP_STATUS_MACROS_IMPL_MP_ASSIGN_OR_RETURN_(                                  \
      ...
      return hand_tracking_mp_lean::StatusBuilder(                                         \
...
#define MP_STATUS_MACROS_IMPL_MP_ASSIGN_OR_RETURN_3_(lhs, rexpr,               \
  ...
  MP_STATUS_MACROS_IMPL_MP_ASSIGN_OR_RETURN_(                                  \
      MP_STATUS_MACROS_IMPL_CONCAT_(_status_or_value, __LINE__), lhs, rexpr,   \
      hand_tracking_mp_lean::StatusBuilder _(                                              \
          std::move(MP_STATUS_MACROS_IMPL_CONCAT_(_status_or_value, __LINE__)) \
              .status(),                                                       \
          MEDIAPIPE_LOC);                                                      \
