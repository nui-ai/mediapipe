...
TEST(CalculatorTest, CanReadWritePortsAndUseServices) {
  ...
  MP_ASSERT_OK(graph.StartRun(
      {{"side_in", hand_tracking_mp_lean::MakePacket<std::string>("side")}}));
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "in", hand_tracking_mp_lean::MakePacket<int>(42).At(Timestamp(0))));
...
TEST(CalculatorTest, CanUseSharedContract) {
  ...
  hand_tracking_mp_lean::Packet out_a;
  MP_ASSERT_OK(
      graph.ObserveOutputStream("out_a", [&out_a](const hand_tracking_mp_lean::Packet& p) {
        out_a = p;
        return absl::OkStatus();
      }));
  hand_tracking_mp_lean::Packet out_b;
  MP_ASSERT_OK(
      graph.ObserveOutputStream("out_b", [&out_b](const hand_tracking_mp_lean::Packet& p) {
        out_b = p;
        return absl::OkStatus();
      }));
  ...
  // Verifying outputs.
  MP_ASSERT_OK_AND_ASSIGN(hand_tracking_mp_lean::Packet side_out_a,
                          graph.GetOutputSidePacket("side_out_a"));
  ASSERT_FALSE(side_out_a.IsEmpty());
  EXPECT_EQ(side_out_a.Get<std::string>(), "side");

  MP_ASSERT_OK_AND_ASSIGN(hand_tracking_mp_lean::Packet side_out_b,
...
TEST(CalculatorTest, TimestampOffsetZeroIsTheDefault) {
  ...
  CalculatorGraph graph;
  MP_ASSERT_OK(graph.Initialize(std::move(config)));
  std::vector<hand_tracking_mp_lean::Packet> output_packets;
  MP_ASSERT_OK(graph.ObserveOutputStream(
      "out",
      [&output_packets](const hand_tracking_mp_lean::Packet& p) {
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "in", hand_tracking_mp_lean::MakePacket<int>(43).At(Timestamp(1))));
  MP_ASSERT_OK(graph.WaitUntilIdle());
...
TEST(CalculatorTest, CanRunGeneratorCalculator) {
  ...
  // Verifying outputs.
  MP_ASSERT_OK_AND_ASSIGN(hand_tracking_mp_lean::Packet value,
