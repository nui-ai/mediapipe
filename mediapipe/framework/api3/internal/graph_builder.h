...
namespace hand_tracking_mp_lean::api3::builder {
...
OptionsT& GetOptions(std::optional<hand_tracking_mp_lean::MediaPipeOptions>& options) {
  if (!options.has_value()) {
    options = hand_tracking_mp_lean::MediaPipeOptions();
  }
...
class Executor {
 public:
  template <typename OptionsT>
  OptionsT& GetOptions() {
    return ::hand_tracking_mp_lean::api3::builder::GetOptions<OptionsT>(options_);
  }
 ...
 private:
  ...
  std::optional<hand_tracking_mp_lean::MediaPipeOptions> options_;
...
class PacketGeneratorBuilder {
 ...
 private:
  ...
  TagIndexMap<SideDestination> in_sides_;
  TagIndexMap<SideSource> out_sides_;
  hand_tracking_mp_lean::PacketGeneratorOptions options_;
