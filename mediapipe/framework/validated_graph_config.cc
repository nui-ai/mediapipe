...
std::string DebugName(const CalculatorGraphConfig& config,
  ...
  switch (node_type) {
    case NodeTypeInfo::NodeType::CALCULATOR:
      return hand_tracking_mp_lean::DebugName(config.node(node_index));
...
absl::Status NodeTypeInfo::Initialize(
  ...
  for (const hand_tracking_mp_lean::protobuf::Any& options : node.node_options()) {
  ...
  if (!status.ok()) {
    statuses.push_back(
        hand_tracking_mp_lean::StatusBuilder(std::move(status), MEDIAPIPE_LOC).SetPrepend()
        << "For input streams ");
  }
...
absl::Status ValidatedGraphConfig::AddOutputSidePacketsForNode(
  ...
  for (CollectionItemId id = tag_map.BeginId(); id < tag_map.EndId(); ++id) {
    ...
    if (!hand_tracking_mp_lean::InsertIfNotPresent(&side_packet_to_producer_, name,
    ...
    if (hand_tracking_mp_lean::ContainsKey(required_side_packets_, name)) {
      if (need_sorting_ptr) {
...
absl::Status ValidatedGraphConfig::AddOutputStream(NodeTypeInfo::NodeRef node,
  ...
  if (!hand_tracking_mp_lean::InsertIfNotPresent(&stream_to_producer_, name,
  ...
  for (const EdgeInfo& stream : input_streams_) {
    ...
    if (!stream.packet_type->IsConsistentWith(
      ...
      return absl::UnknownError(absl::Substitute(
          ...
          "\"$3\"",
          stream.name,
          hand_tracking_mp_lean::DebugName(config_.node(stream.parent_node.index)),
...
absl::Status ValidatedGraphConfig::ValidateRequiredSidePackets(
  ...
  for (const auto& required_item : required_side_packets_) {
    ...
    if (iter == side_packets.end()) {
      ...
      statuses.push_back(hand_tracking_mp_lean::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
    ...
    for (int index : required_item.second) {
      ...
      if (!status.ok()) {
        statuses.push_back(
            hand_tracking_mp_lean::StatusBuilder(std::move(status), MEDIAPIPE_LOC)
                .SetPrepend()
            << "Side packet \"" << required_item.first
            << "\" failed validation: ");
      }
