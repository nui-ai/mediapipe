...
std::shared_ptr<cv::Mat> MatView(const hand_tracking_mp_lean::Image* image) {
  ...
  struct MatWithPixelLock {
    ...
    // std::make_shared.
    MatWithPixelLock(hand_tracking_mp_lean::Image* image) : lock(image) {}
    hand_tracking_mp_lean::PixelWriteLock lock;
    cv::Mat mat;
  };
  ...
  const size_t steps[] = {static_cast<size_t>(image->step()),
                          static_cast<size_t>(ImageFrame::ByteDepthForFormat(
                              image->image_format()))};
  auto owner =
      std::make_shared<MatWithPixelLock>(const_cast<hand_tracking_mp_lean::Image*>(image));
