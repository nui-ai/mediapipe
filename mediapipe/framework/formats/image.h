...
class Image {
 public:
  ...
  explicit Image(CFHolder<CVPixelBufferRef> pixel_buffer)
      : Image(hand_tracking_mp_lean::GpuBuffer(std::move(pixel_buffer))) {}
  explicit Image(CVPixelBufferRef pixel_buffer)
      : Image(hand_tracking_mp_lean::GpuBuffer(pixel_buffer)) {}
#else
  explicit Image(hand_tracking_mp_lean::GlTextureBufferSharedPtr texture_buffer)
      : Image(hand_tracking_mp_lean::GpuBuffer(std::move(texture_buffer))) {}
#endif  // MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
  explicit Image(hand_tracking_mp_lean::GpuBuffer gpu_buffer) {
    use_gpu_ = true;
  ...
  CVPixelBufferRef GetCVPixelBufferRef() const {
    ...
    return hand_tracking_mp_lean::GetCVPixelBufferRef(gpu_buffer_);
  }
#else
  hand_tracking_mp_lean::GlTextureBufferSharedPtr GetGlTextureBufferSharedPtr() const {
    if (use_gpu_ == false) ConvertToGpu();
    return gpu_buffer_.internal_storage<hand_tracking_mp_lean::GlTextureBuffer>();
  }
#endif  // MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
#endif  // !MEDIAPIPE_DISABLE_GPU
  ...
  const hand_tracking_mp_lean::GpuBuffer GetGpuBuffer(bool upload_to_gpu = true) const {
  ...
  hand_tracking_mp_lean::GpuBufferFormat format() const;
 ...
 private:
  mutable hand_tracking_mp_lean::GpuBuffer gpu_buffer_;
...
inline ImageFormat::Format Image::image_format() const {
  return hand_tracking_mp_lean::ImageFormatForGpuBufferFormat(gpu_buffer_.format());
}

inline hand_tracking_mp_lean::GpuBufferFormat Image::format() const {
