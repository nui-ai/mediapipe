...
class GraphProfiler : public std::enable_shared_from_this<ProfilingContext> {
 public:
  ...
  void SetClock(const std::shared_ptr<hand_tracking_mp_lean::Clock>& clock)
      ABSL_LOCKS_EXCLUDED(profiler_mutex_);

  // Gets the profiler clock.
  const std::shared_ptr<hand_tracking_mp_lean::Clock> GetClock() const
      ABSL_LOCKS_EXCLUDED(profiler_mutex_);
  ...
  absl::Status Start(hand_tracking_mp_lean::Executor* executor);
