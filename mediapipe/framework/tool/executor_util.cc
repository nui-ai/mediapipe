...
void EnsureMinimumDefaultExecutorStackSize(const int32_t min_stack_size,
  ...
  hand_tracking_mp_lean::ExecutorConfig* default_executor_config = nullptr;
  for (hand_tracking_mp_lean::ExecutorConfig& executor_config :
  ...
  if (!default_executor_config) {
    ...
    if (config->num_threads()) {
      default_executor_config->mutable_options()
          ->MutableExtension(hand_tracking_mp_lean::ThreadPoolExecutorOptions::ext)
  ...
  if (default_executor_config->type().empty() ||
    ...
    hand_tracking_mp_lean::ThreadPoolExecutorOptions* extension =
        default_executor_config->mutable_options()->MutableExtension(
            hand_tracking_mp_lean::ThreadPoolExecutorOptions::ext);
