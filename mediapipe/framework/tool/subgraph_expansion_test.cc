...
class NodeChainSubgraph : public Subgraph {
 public:
  absl::StatusOr<CalculatorGraphConfig> GetConfig(
      const SubgraphOptions& options) override {
    auto opts =
        Subgraph::GetOptions<hand_tracking_mp_lean::NodeChainSubgraphOptions>(options);
...
TEST(SubgraphExpansionTest, TransformStreamNames) {
  ...
  EXPECT_THAT(config, hand_tracking_mp_lean::EqualsProto(expected_config));
}
...
TEST(SubgraphExpansionTest, ConnectSubgraphStreams) {
  ...
  EXPECT_THAT(subgraph, hand_tracking_mp_lean::EqualsProto(expected_subgraph));
}
...
TEST(SubgraphExpansionTest, ExpandSubgraphs) {
  ...
  EXPECT_THAT(supergraph, hand_tracking_mp_lean::EqualsProto(expected_graph));
}
...
TEST(SubgraphExpansionTest, ExecutorFieldOfNodeInSubgraphPreserved) {
  ...
  CalculatorGraphConfig expected_graph = hand_tracking_mp_lean::ParseTextProtoOrDie<
...
TEST(SubgraphExpansionTest, SubgraphOptionsUsage) {
  ...
  auto graph_options =
      hand_tracking_mp_lean::ParseTextProtoOrDie<Subgraph::SubgraphOptions>(R"pb(
        options {
  ...
  EXPECT_THAT(subgraph, hand_tracking_mp_lean::EqualsProto(expected_graph));
}
...
TEST(SubgraphExpansionTest, SimpleSubgraphOptionsUsage) {
  ...
  EXPECT_THAT(moon_subgraph, hand_tracking_mp_lean::EqualsProto(expected_graph));
}
...
TEST(SubgraphExpansionTest, SimpleSubgraphOptionsTwice) {
  ...
  auto sky_graph = hand_tracking_mp_lean::ParseTextProtoOrDie<CalculatorGraphConfig>(R"pb(
    graph_options: {
      [type.googleapis.com/mediapipe.NodeChainSubgraphOptions] {}
    }
    node: {
  ...
  auto expected_node_options =
      hand_tracking_mp_lean::ParseTextProtoOrDie<hand_tracking_mp_lean::NodeChainSubgraphOptions>(
          "chain_length: 3");
  hand_tracking_mp_lean::NodeChainSubgraphOptions node_options;
  sky_graph.node(0).node_options(0).UnpackTo(&node_options);
  ASSERT_THAT(node_options, hand_tracking_mp_lean::EqualsProto(expected_node_options));
  ...
  EXPECT_THAT(expanded_1, hand_tracking_mp_lean::EqualsProto(expected_graph));
  EXPECT_THAT(sky_graph, hand_tracking_mp_lean::EqualsProto(expected_graph));
}
...
class InternalExecutorSubgraph : public Subgraph {
 public:
  absl::StatusOr<CalculatorGraphConfig> GetConfig(
      const SubgraphOptions& options) override {
    return hand_tracking_mp_lean::ParseTextProtoOrDie<CalculatorGraphConfig>(R"pb(
      input_stream: "IN:foo"
