...
struct HasProcessMethod<
    ...
    std::void_t<decltype(absl::Status(std::declval<std::decay_t<T>>().Process(
        std::declval<hand_tracking_mp_lean::CalculatorContext*>())))>> : std::true_type {};
...
struct TagTuple {
  ...
  static constexpr auto Make(std::index_sequence<I...> indices) {
    return std::make_tuple(hand_tracking_mp_lean::api2::internal::tag_build(S<I>{})...);
  }
...
class Contract {
 public:
  ...
  absl::Status GetContract(hand_tracking_mp_lean::CalculatorContract* cc) const {
