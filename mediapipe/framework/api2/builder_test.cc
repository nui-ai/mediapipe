...
using ::hand_tracking_mp_lean::api2::test::Bar;
using ::hand_tracking_mp_lean::api2::test::FloatAdder;
using ::hand_tracking_mp_lean::api2::test::Foo;
using ::hand_tracking_mp_lean::api2::test::Foo2;
using ::hand_tracking_mp_lean::api2::test::FooBar1;
...
TEST(BuilderTest, AnyAndSameTypeHandledProperly) {
  ...
  CalculatorGraphConfig expected = hand_tracking_mp_lean::ParseTextProtoOrDie<
...
TEST(GetOptionsTest, AddProto3Options) {
  ...
  auto& foo = graph.AddNode("Foo");
  foo.GetOptions<hand_tracking_mp_lean::SkyLightCalculatorOptions>();
  base >> foo.In("BASE");
  side >> foo.SideIn("SIDE");
  Stream<AnyType> foo_out = foo.Out("OUT");
...
TEST(GetOptionsTest, AddProto2Options) {
  ...
  auto& foo = graph.AddNode("Foo");
  foo.GetOptions<hand_tracking_mp_lean::NightLightCalculatorOptions>();
  base >> foo.In("BASE");
  side >> foo.SideIn("SIDE");
  Stream<AnyType> foo_out = foo.Out("OUT");
