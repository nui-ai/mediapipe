  ...
  if (cc != nullptr) {
    ...
    for (const auto& [key, packet] : parent_service_packets) {
      ...
      if (key == kGpuService.key) {
        ...
        auto resources = hand_tracking_mp_lean::GpuResources::Create(
            *packet.Get<std::shared_ptr<hand_tracking_mp_lean::GpuResources>>());
        ABSL_CHECK_OK(resources);
        service_packets[key] =
            MakePacket<std::shared_ptr<hand_tracking_mp_lean::GpuResources>>(*resources);
        continue;
      }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status CalculatorGraph::InitializeExecutors() {
  ...
  for (const ExecutorConfig& executor_config :
    ...
    if (hand_tracking_mp_lean::ContainsKey(executors_, executor_config.name())) {
  ...
  if (!hand_tracking_mp_lean::ContainsKey(executors_, "")) {
...
absl::Status CalculatorGraph::InitializeDefaultExecutor(
  ...
  if (num_threads == 0 || num_threads == -1) {
    num_threads = std::min(
        hand_tracking_mp_lean::NumCPUCores(),
...
absl::Status CalculatorGraph::ObserveOutputStream(
  ...
  if (output_stream_index < 0) {
    return hand_tracking_mp_lean::NotFoundErrorBuilder(MEDIAPIPE_LOC)
...
absl::StatusOr<Packet> CalculatorGraph::GetOutputSidePacket(
  ...
  if (output_packet.IsEmpty()) {
    ...
    if (base_packet_iter != base_packets.end() &&
      ...
      return hand_tracking_mp_lean::UnavailableErrorBuilder(MEDIAPIPE_LOC)
...
std::shared_ptr<::hand_tracking_mp_lean::GpuResources> CalculatorGraph::GetGpuResources()
    const {
...
absl::Status CalculatorGraph::MaybeSetUpGpuServiceFromLegacySidePacket(
  ...
  if (gpu_resources) {
    ABSL_LOG(WARNING)
        << "::hand_tracking_mp_lean::GpuSharedData provided as a side packet while the "
        << "graph already had one; ignoring side packet";
    return absl::OkStatus();
  }
  gpu_resources = legacy_sp.Get<::hand_tracking_mp_lean::GpuSharedData*>()->gpu_resources;
...
std::map<std::string, Packet> CalculatorGraph::MaybeCreateLegacyGpuSidePacket(
  ...
  if (gpu_resources &&
      (legacy_sp.IsEmpty() ||
       legacy_sp.Get<::hand_tracking_mp_lean::GpuSharedData*>()->gpu_resources !=
           gpu_resources)) {
    legacy_gpu_shared_ =
        std::make_unique<hand_tracking_mp_lean::GpuSharedData>(gpu_resources);
    additional_side_packets[kGpuSharedSidePacketName] =
        MakePacket<::hand_tracking_mp_lean::GpuSharedData*>(legacy_gpu_shared_.get());
  }
...
absl::Status CalculatorGraph::PrepareForRun(
  ...
  for (const auto& name_max : graph_input_stream_max_queue_size_) {
    std::unique_ptr<GraphInputStream>* stream =
        hand_tracking_mp_lean::FindOrNull(graph_input_streams_, name_max.first);
...
absl::StatusOr<GraphRuntimeInfo> CalculatorGraph::GetGraphRuntimeInfo() {
  ...
  const absl::Time time_now = hand_tracking_mp_lean::Clock::RealClock()->TimeNow();
...
absl::Status CalculatorGraph::SetInputStreamTimestampBound(
  ...
  std::unique_ptr<GraphInputStream>* stream =
      hand_tracking_mp_lean::FindOrNull(graph_input_streams_, stream_name);
  RET_CHECK(stream).SetNoLogging() << absl::Substitute(
...
bool CalculatorGraph::HasInputStream(const std::string& stream_name) {
  return hand_tracking_mp_lean::FindOrNull(graph_input_streams_, stream_name) != nullptr;
}
...
void CalculatorGraph::CallStatusHandlers(GraphRunState graph_run_state,
  ...
  for (int status_handler_index = 0;
    ...
    if (!packet_set_statusor.ok()) {
      RecordError(hand_tracking_mp_lean::StatusBuilder(
    ...
    if (!handler_result.ok()) {
      hand_tracking_mp_lean::StatusBuilder builder(std::move(handler_result),
                                       MEDIAPIPE_LOC);
...

  {
    ...
    if (*stream_was_full != stream_is_full) {
      for (int node_id : *upstream_nodes) {
        ...
        hand_tracking_mp_lean::LogEvent(profiler_.get(),
