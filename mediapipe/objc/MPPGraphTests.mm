...
class GrayscaleCalculator : public Calculator {
 public:
  ...
  absl::Status Process() final {
    ...
    auto output = absl::make_unique<hand_tracking_mp_lean::ImageFrame>(ImageFormat::GRAY8, w, h);
...
- (void)testSetStreamHeader {
  ...
  auto header_packet = hand_tracking_mp_lean::MakePacket<int>(0xDEADBEEF);
...
- (void)testRawPacketOutput {
  ...
  _packetOutputBlock = ^(MPPGraph* outputGraph, const hand_tracking_mp_lean::Packet& packet,
                         const std::string& outputStreamName) {
    STRONGIFY(self);
  ...
  XCTAssert([_graph startWithError:nil]);
  XCTAssert([_graph sendPacket:hand_tracking_mp_lean::MakePacket<int>(kTestValue).At(hand_tracking_mp_lean::Timestamp(1))
...
- (void)testPixelBufferToImage {
  ...
  hand_tracking_mp_lean::Packet packet = [_graph imagePacketWithPixelBuffer:*pixelBufferIn];
  CVPixelBufferRef pixelBufferOut = packet.Get<hand_tracking_mp_lean::Image>().GetCVPixelBufferRef();
