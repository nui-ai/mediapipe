...
std::unique_ptr<hand_tracking_mp_lean::Matrix> MakeMatrix(const AudioBuffer* buffers, CMItemCount channels,
                                            CMItemCount frames, bool interleaved) {
  // Create the matrix and fill it accordingly. Its dimensions are `channels x frames`.
  auto matrix = std::make_unique<hand_tracking_mp_lean::Matrix>(channels, frames);
...
absl::StatusOr<std::unique_ptr<hand_tracking_mp_lean::Matrix>> MediaPipeConvertAudioBufferListToAudioMatrix(
