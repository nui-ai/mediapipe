...
absl::Status CreateCVPixelBufferForImageFramePacket(
  ...
  auto image_frame = std::const_pointer_cast<hand_tracking_mp_lean::ImageFrame>(
      hand_tracking_mp_lean::SharedPtrWithPacket<hand_tracking_mp_lean::ImageFrame>(
          image_frame_packet));
...
absl::StatusOr<CFHolder<CVPixelBufferRef>> CreateCVPixelBufferForImageFrame(
    std::shared_ptr<hand_tracking_mp_lean::ImageFrame> image_frame, bool can_overwrite) {
  ...
  hand_tracking_mp_lean::ImageFormat::Format image_format = frame.Format();
...
absl::StatusOr<CFHolder<CVPixelBufferRef>> CreateCVPixelBufferCopyingImageFrame(
  ...
  hand_tracking_mp_lean::ImageFormat::Format image_format = image_frame.Format();
  switch (image_format) {
    case hand_tracking_mp_lean::ImageFormat::SRGBA:
      pixel_format = kCVPixelFormatType_32BGRA;
...
std::unique_ptr<hand_tracking_mp_lean::ImageFrame> CreateImageFrameForCVPixelBuffer(
  ...
  size_t width = CVPixelBufferGetWidth(image_buffer);
  size_t height = CVPixelBufferGetHeight(image_buffer);
  std::unique_ptr<hand_tracking_mp_lean::ImageFrame> frame;

  CVPixelBufferRetain(image_buffer);

  OSType pixel_format = CVPixelBufferGetPixelFormatType(image_buffer);
  hand_tracking_mp_lean::ImageFormat::Format image_format = hand_tracking_mp_lean::ImageFormat::UNKNOWN;
  switch (pixel_format) {
    case kCVPixelFormatType_32BGRA: {
      image_format = hand_tracking_mp_lean::ImageFormat::SRGBA;
      if (!bgr_as_rgb) {
        // Swap R and B channels.
        ...
        if (can_overwrite) {
          ...
          frame = absl::make_unique<hand_tracking_mp_lean::ImageFrame>(image_format, width,
                                                           height);
    ...
    case kCVPixelFormatType_24RGB:
      image_format = hand_tracking_mp_lean::ImageFormat::SRGB;
      break;

    case kCVPixelFormatType_OneComponent8:
      image_format = hand_tracking_mp_lean::ImageFormat::GRAY8;
      break;
  ...
  if (frame) {
    ...
    frame = absl::make_unique<hand_tracking_mp_lean::ImageFrame>(
