...
class CollectionHasMinSizeCalculator : public CalculatorBase {
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    ...
    RET_CHECK_GE(
        cc->Options<::hand_tracking_mp_lean::CollectionHasMinSizeCalculatorOptions>()
            .min_size(),
        0);
  ...
  absl::Status Process(CalculatorContext* cc) override {
    ...
    ABSL_LOG(INFO) << "legacy part has_min_size: " << has_min_size << " (min_size_ is " << min_size_ << ", input iterable size is " << (cc->Inputs().Tag("ITERABLE").IsEmpty() ? 0 : cc->Inputs().Tag("ITERABLE").Get<IterableT>().size()) << ")";
