...
struct LandmarksRefinementNode : Node<kLandmarksRefinementNodeName> {
  ...
  struct Contract {
    ...
    Repeated<Input<S, hand_tracking_mp_lean::NormalizedLandmarkList>> landmarks{
        "LANDMARKS"};
    ...
    // mapping).
    Output<S, hand_tracking_mp_lean::NormalizedLandmarkList> refined_landmarks{
        "REFINED_LANDMARKS"};

    Options<S, hand_tracking_mp_lean::LandmarksRefinementCalculatorOptions> options;
  };
};
