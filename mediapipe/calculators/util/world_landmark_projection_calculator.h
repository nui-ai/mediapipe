...
struct WorldLandmarkProjectionNode : Node<kWorldLandmarkProjectionNodeName> {
  ...
  struct Contract {
    ...
    Input<S, hand_tracking_mp_lean::LandmarkList> input_landmarks{"LANDMARKS"};

    // An NormalizedRect representing a normalized rectangle in image
    // coordinates.
    Optional<Input<S, hand_tracking_mp_lean::NormalizedRect>> input_rect{"NORM_RECT"};
    ...
    Output<S, hand_tracking_mp_lean::LandmarkList> output_landmarks{"LANDMARKS"};
  };
};
