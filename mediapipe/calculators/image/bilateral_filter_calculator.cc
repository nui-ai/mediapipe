...
class BilateralFilterCalculator : public CalculatorBase {
 ...
 private:
  ...
  hand_tracking_mp_lean::BilateralFilterCalculatorOptions options_;
...
absl::Status BilateralFilterCalculator::GetContract(CalculatorContract* cc) {
...
#if !MEDIAPIPE_DISABLE_GPU
  if (cc->Inputs().HasTag(kInputFrameTagGpu)) {
    cc->Inputs().Tag(kInputFrameTagGpu).Set<hand_tracking_mp_lean::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
  if (cc->Inputs().HasTag(kInputFrameTag)) {
  ...
  if (cc->Inputs().HasTag(kInputGuideTagGpu)) {
    cc->Inputs().Tag(kInputGuideTagGpu).Set<hand_tracking_mp_lean::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
  ...
  if (cc->Outputs().HasTag(kOutputFrameTagGpu)) {
    cc->Outputs().Tag(kOutputFrameTagGpu).Set<hand_tracking_mp_lean::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status BilateralFilterCalculator::Open(CalculatorContext* cc) {
  ...
  options_ = cc->Options<hand_tracking_mp_lean::BilateralFilterCalculatorOptions>();
...
absl::Status BilateralFilterCalculator::RenderCpu(CalculatorContext* cc) {
  ...
  auto input_mat = hand_tracking_mp_lean::formats::MatView(&input_frame);
  ...
  if (has_guide_image) {
    ...
    auto output_mat = hand_tracking_mp_lean::formats::MatView(output_frame.get());
...
absl::Status BilateralFilterCalculator::RenderGpu(CalculatorContext* cc) {
  ...
  const auto& input_frame =
      cc->Inputs().Tag(kInputFrameTagGpu).Get<hand_tracking_mp_lean::GpuBuffer>();
  ...
  if (has_guide_image) {
    ...
    const auto& guide_image =
        cc->Inputs().Tag(kInputGuideTagGpu).Get<hand_tracking_mp_lean::GpuBuffer>();
  ...
  auto output_frame = output_texture.GetFrame<hand_tracking_mp_lean::GpuBuffer>();
  cc->Outputs()
...
absl::Status BilateralFilterCalculator::GlSetup(CalculatorContext* cc) {
  ...
  if (has_guide_image) {
    ...
    hand_tracking_mp_lean::GlhCreateProgram(
        hand_tracking_mp_lean::kBasicVertexShader, joint_frag_src.c_str(), NUM_ATTRIBUTES,
  ...
  glBufferData(GL_ARRAY_BUFFER, 4 * 2 * sizeof(GLfloat),
               hand_tracking_mp_lean::kBasicSquareVertices, GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, 4 * 2 * sizeof(GLfloat),
               hand_tracking_mp_lean::kBasicTextureVertices, GL_STATIC_DRAW);
