...
class SetAlphaCalculator : public CalculatorBase {
 ...
 private:
  ...
  hand_tracking_mp_lean::SetAlphaCalculatorOptions options_;
...
absl::Status SetAlphaCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->Inputs().HasTag(kInputAlphaTagGpu)) {
    cc->Inputs().Tag(kInputAlphaTagGpu).Set<hand_tracking_mp_lean::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status SetAlphaCalculator::Open(CalculatorContext* cc) {
  ...
  options_ = cc->Options<hand_tracking_mp_lean::SetAlphaCalculatorOptions>();
...
absl::Status SetAlphaCalculator::RenderGpu(CalculatorContext* cc) {
  ...
  if (!(input_frame.format() == hand_tracking_mp_lean::GpuBufferFormat::kBGRA32 ||
        input_frame.format() == hand_tracking_mp_lean::GpuBufferFormat::kRGB24)) {
  ...
  auto output_texture = gpu_helper_.CreateDestinationTexture(
      width, height, hand_tracking_mp_lean::GpuBufferFormat::kBGRA32);
  ...
  if (has_alpha_mask) {
    const auto& alpha_mask =
        cc->Inputs().Tag(kInputAlphaTagGpu).Get<hand_tracking_mp_lean::GpuBuffer>();
...
absl::Status SetAlphaCalculator::GlSetup(CalculatorContext* cc) {
  ...
  hand_tracking_mp_lean::GlhCreateProgram(hand_tracking_mp_lean::kBasicVertexShader, frag_src,
