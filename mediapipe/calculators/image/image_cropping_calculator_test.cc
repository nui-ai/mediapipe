...
std::unique_ptr<hand_tracking_mp_lean::ImageFrame> GetInputFrame(int width, int height,
                                                     int channel) {
  const int total_size = width * height * channel;

  auto image_format = channel == 4 ? hand_tracking_mp_lean::ImageFormat::SRGBA
                                   : hand_tracking_mp_lean::ImageFormat::SRGB;

  auto input_frame = std::make_unique<hand_tracking_mp_lean::ImageFrame>(
...
TEST(ImageCroppingCalculatorTest, IdentityFunctionCropWithOriginalSize) {
  auto calculator_node =
      ParseTextProtoOrDie<hand_tracking_mp_lean::CalculatorGraphConfig::Node>(
          absl::Substitute(
              R"pb(
  ...
  const auto& output_image =
      outputs.Tag("IMAGE").packets[0].Get<hand_tracking_mp_lean::ImageFrame>();
...
TEST(ImageCroppingCalculatorTest, IdentityFunctionCropWithOriginalSizeGPU) {
  hand_tracking_mp_lean::CalculatorGraphConfig config =
      ParseTextProtoOrDie<hand_tracking_mp_lean::CalculatorGraphConfig>(absl::Substitute(
          R"pb(
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "input_frames", input_frame_packet.At(hand_tracking_mp_lean::Timestamp(1))));
