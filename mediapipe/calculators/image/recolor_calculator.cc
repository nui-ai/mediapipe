...
class RecolorCalculator : public CalculatorBase {
 ...
 private:
  ...
  hand_tracking_mp_lean::RecolorCalculatorOptions::MaskChannel mask_channel_;
...
absl::Status RecolorCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->Inputs().HasTag(kMaskGpuTag)) {
    cc->Inputs().Tag(kMaskGpuTag).Set<hand_tracking_mp_lean::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status RecolorCalculator::RenderCpu(CalculatorContext* cc) {
  ...
  if (mask_mat.channels() > 1) {
    ...
    if (mask_channel_ == hand_tracking_mp_lean::RecolorCalculatorOptions_MaskChannel_ALPHA)
      mask_mat = channels[3];
    else
      mask_mat = channels[0];
  }
  ...
  cv::Mat output_mat = hand_tracking_mp_lean::formats::MatView(output_img.get());
...
absl::Status RecolorCalculator::RenderGpu(CalculatorContext* cc) {
  ...
  const auto& mask_buffer = mask_packet.Get<hand_tracking_mp_lean::GpuBuffer>();
...
absl::Status RecolorCalculator::LoadOptions(CalculatorContext* cc) {
  const auto& options = cc->Options<hand_tracking_mp_lean::RecolorCalculatorOptions>();
...
absl::Status RecolorCalculator::InitGpu(CalculatorContext* cc) {
  ...
  switch (mask_channel_) {
    case hand_tracking_mp_lean::RecolorCalculatorOptions_MaskChannel_UNKNOWN:
    case hand_tracking_mp_lean::RecolorCalculatorOptions_MaskChannel_RED:
      mask_component = "r";
      break;
    case hand_tracking_mp_lean::RecolorCalculatorOptions_MaskChannel_ALPHA:
      mask_component = "a";
      break;
  }
