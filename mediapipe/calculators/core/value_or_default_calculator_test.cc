...
class ValueOrDefaultRunner : public hand_tracking_mp_lean::CalculatorRunner {
 public:
  ValueOrDefaultRunner()
      : hand_tracking_mp_lean::CalculatorRunner(R"pb(
    ...
    MutableSidePackets()->Index(0) = hand_tracking_mp_lean::MakePacket<int>(kDefaultValue);
  }
  ...
  void TickAt(int64_t time) {
    ...
    MutableInputs()->Tag("TICK").packets.push_back(
        hand_tracking_mp_lean::Adopt(new bool(false)).At(hand_tracking_mp_lean::Timestamp(time)));
  }
  ...
  void ProvideInput(int64_t time, int value) {
    MutableInputs()->Tag("IN").packets.push_back(
        hand_tracking_mp_lean::Adopt(new int(value)).At(hand_tracking_mp_lean::Timestamp(time)));
  }

  // Extracts the timestamps (as int64) of the output stream of the calculator.
  std::vector<int64_t> GetOutputTimestamps() const {
    std::vector<int64_t> timestamps;
    for (const hand_tracking_mp_lean::Packet& packet : Outputs().Tag("OUT").packets) {
  ...
  std::vector<int64_t> GetFlagTimestamps() const {
    std::vector<int64_t> timestamps;
    for (const hand_tracking_mp_lean::Packet& packet : Outputs().Tag("FLAG").packets) {
      timestamps.emplace_back(packet.Timestamp().Value());
    }
    return timestamps;
  }
