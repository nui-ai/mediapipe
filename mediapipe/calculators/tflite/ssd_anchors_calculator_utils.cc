...
absl::StatusOr<std::vector<Anchor>> SsdAnchorsCalculatorUtils::GenerateAnchorsFromConfigFile(
    const std::string& config_path) {
  SsdAnchorsCalculatorOptions options;
  MP_RETURN_IF_ERROR(LoadOptionsFromFile(config_path, &options));
  std::vector<Anchor> anchors;
  MP_RETURN_IF_ERROR(GenerateAnchors(&anchors, options));
  return anchors;
}

absl::Status SsdAnchorsCalculatorUtils::LoadOptionsFromFile(
    const std::string& config_path,
    SsdAnchorsCalculatorOptions* options) {
  std::string file_contents;
  MP_RETURN_IF_ERROR(file::GetContents(config_path, &file_contents));

  std::istringstream config_stream(file_contents);
  std::string line;
  ...
  while (std::getline(config_stream, line)) {
    ...
    if (key == "num_layers") {
      options->set_num_layers(std::stoi(value_str));
    } else if (key == "min_scale") {
      options->set_min_scale(std::stof(value_str));
    } else if (key == "max_scale") {
      options->set_max_scale(std::stof(value_str));
    } else if (key == "input_size_width") {
      options->set_input_size_width(std::stoi(value_str));
    } else if (key == "input_size_height") {
      options->set_input_size_height(std::stoi(value_str));
    } else if (key == "anchor_offset_x") {
      options->set_anchor_offset_x(std::stof(value_str));
    } else if (key == "anchor_offset_y") {
      options->set_anchor_offset_y(std::stof(value_str));
    } else if (key == "fixed_anchor_size") {
      options->set_fixed_anchor_size(value_str == "true");
    } else if (key == "strides") {
      ...
      std::istringstream stride_stream(value_str);
      std::string stride_val;
      while (std::getline(stride_stream, stride_val, ',')) {
        stride_val.erase(0, stride_val.find_first_not_of(" \t"));
        stride_val.erase(stride_val.find_last_not_of(" \t") + 1);
        if (!stride_val.empty()) {
          options->add_strides(std::stoi(stride_val));
        }
      }
      ...
      // Parse comma-separated list
      std::istringstream ratio_stream(value_str);
      std::string ratio_val;
      while (std::getline(ratio_stream, ratio_val, ',')) {
        ratio_val.erase(0, ratio_val.find_first_not_of(" \t"));
        ratio_val.erase(ratio_val.find_last_not_of(" \t") + 1);
        if (!ratio_val.empty()) {
          options->add_aspect_ratios(std::stof(ratio_val));
        }
      }
    }
  }
...
absl::Status SsdAnchorsCalculatorUtils::GenerateMultiScaleAnchors(
