...
void RunTestWithInputImagePacket(
    ...
    const hand_tracking_mp_lean::NormalizedRect& roi, bool output_int_tensor,
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "roi",
      MakePacket<hand_tracking_mp_lean::NormalizedRect>(std::move(roi)).At(Timestamp(0))));
...
Packet MakeImagePacket(cv::Mat input) {
  hand_tracking_mp_lean::Image input_image(std::make_shared<hand_tracking_mp_lean::ImageFrame>(
      ...
      input.data, [](uint8_t*) {}));
  return MakePacket<hand_tracking_mp_lean::Image>(std::move(input_image)).At(Timestamp(0));
}
...
void RunTest(cv::Mat input, cv::Mat expected_result,
             ...
             const hand_tracking_mp_lean::NormalizedRect& roi) {
