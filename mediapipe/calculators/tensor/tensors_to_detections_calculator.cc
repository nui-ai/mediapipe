...
namespace api2 {
  ...
  absl::Status TensorsToDetectionsCalculator::ProcessGPU(
    ...
    MP_RETURN_IF_ERROR(gpu_helper_.RunInGlContext([this, &input_tensors, &cc,
                                                   &output_detections]()
                                                      -> absl::Status {
      if (!anchors_init_) {
        ABSL_LOG(WARNING) << "Initializing SSD anchors from GPU buffer, not sure why this should work or be useful.";
        if (input_tensors.size() == kNumInputTensorsWithAnchors) {
          auto read_view = input_tensors[ssd_decoding_tensor_mapping_.anchors_tensor_index()]
          ...
          glCopyBufferSubData(
              ...
              input_tensors[ssd_decoding_tensor_mapping_.anchors_tensor_index()].bytes());
      ...
      // the reading buffers' views.
      {
        ...
        auto input1_view = input_tensors[ssd_decoding_tensor_mapping_.scores_tensor_index()]
                               .GetOpenGlBufferReadView();
    ...
    if (!anchors_init_) {
      if (input_tensors.size() == kNumInputTensorsWithAnchors) {
        ...
        auto src_buffer = MtlBufferView::GetReadView(
            input_tensors[ssd_decoding_tensor_mapping_.anchors_tensor_index()],
            command_buffer);
        auto dest_buffer =
    ...
    [command_encoder setComputePipelineState:decode_program_];
    {
      ...
      auto input1_view = MtlBufferView::GetReadView(
          input_tensors[ssd_decoding_tensor_mapping_.scores_tensor_index()], command_buffer);
  ...
  absl::Status TensorsToDetectionsCalculator::SetSsdAnchors() {
  ...
  absl::Status TensorsToDetectionsCalculator::SetSsdDecodingOptions(CalculatorContext* cc) {
    ...
    // Setup class index set for filtering
    if (kSideInIgnoreClasses(cc).IsConnected()) {
      RET_CHECK(!kSideInIgnoreClasses(cc).IsEmpty());
      RET_CHECK(ssd_decoding_options_.allow_classes().empty());
    ...
    } else if (!ssd_decoding_options_.allow_classes().empty()) {
      RET_CHECK(ssd_decoding_options_.ignore_classes().empty());
      class_index_set_.is_allowlist = true;
      for (int i = 0; i < ssd_decoding_options_.allow_classes_size(); ++i) {
        class_index_set_.values.insert(ssd_decoding_options_.allow_classes(i));
      }
    } else {
      class_index_set_.is_allowlist = false;
      for (int i = 0; i < ssd_decoding_options_.ignore_classes_size(); ++i) {
        class_index_set_.values.insert(ssd_decoding_options_.ignore_classes(i));
      }
    }
