...
absl::StatusOr<bool> ShouldFlipVertically(
    const hand_tracking_mp_lean::TensorConverterCalculatorOptions& options, bool use_gpu) {
  ...
  return hand_tracking_mp_lean::IsGpuOriginAtBottom(options.gpu_origin());
}
...
absl::Status TensorConverterCalculator::InitGpu(CalculatorContext* cc) {
  ...
  const auto& input =
      cc->Inputs().Tag(kGpuBufferTag).Get<hand_tracking_mp_lean::GpuBuffer>();
  hand_tracking_mp_lean::GpuBufferFormat format = input.format();
  const bool include_alpha = (max_num_channels_ == 4);
  const bool single_channel = (max_num_channels_ == 1);

  RET_CHECK(format == hand_tracking_mp_lean::GpuBufferFormat::kBGRA32 ||
            format == hand_tracking_mp_lean::GpuBufferFormat::kRGB24 ||
            format == hand_tracking_mp_lean::GpuBufferFormat::kRGBA32 ||
            format == hand_tracking_mp_lean::GpuBufferFormat::kRGBAFloat128 ||
            format == hand_tracking_mp_lean::GpuBufferFormat::kRGBAHalf64 ||
            format == hand_tracking_mp_lean::GpuBufferFormat::kGrayFloat32 ||
            format == hand_tracking_mp_lean::GpuBufferFormat::kGrayHalf16 ||
            format == hand_tracking_mp_lean::GpuBufferFormat::kOneComponent8)
...
absl::Status TensorConverterCalculator::LoadOptions(CalculatorContext* cc,
  ...
  const auto& options =
      cc->Options<::hand_tracking_mp_lean::TensorConverterCalculatorOptions>();
