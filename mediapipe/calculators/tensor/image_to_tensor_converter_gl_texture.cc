...
class ImageToTensorGlTextureConverter : public ImageToTensorConverter {
 public:
  absl::Status Init(CalculatorContext* cc, bool input_starts_at_bottom,
    ...
    return gl_helper_.RunInGlContext([this, input_starts_at_bottom,
      ...
      const std::string extract_sub_rect_vertex_src =
          absl::StrCat(hand_tracking_mp_lean::kMediaPipeVertexShaderPreamble,
      ...
      hand_tracking_mp_lean::GlhCreateProgram(extract_sub_rect_vertex_src.c_str(),
  ...
  absl::Status ExtractSubRect(const hand_tracking_mp_lean::GlTexture& texture,
    ...
    const auto gl_context = hand_tracking_mp_lean::GlContext::GetCurrent();
    ABSL_LOG_IF(FATAL, !gl_context) << "GlContext is not bound to the thread.";
    if (gl_context->GetGlVersion() == hand_tracking_mp_lean::GlVersion::kGLES2) {
