...
// Usage example:
// node {
//   calculator: "TensorsToFloatsCalculatorNew"
//   input_stream: "TENSORS:tensors"
//   output_stream: "FLOATS:floats"
// }
namespace api2 {
class TensorsToFloatsCalculatorNew : public Node {
 public:
  ...
  MEDIAPIPE_NODE_INTERFACE(TensorsToFloatsCalculatorNew, kInTensors, kOutFloat,
                           kOutFloats);
...
MEDIAPIPE_REGISTER_NODE(TensorsToFloatsCalculatorNew);

absl::Status TensorsToFloatsCalculatorNew::UpdateContract(CalculatorContract* cc) {
  // Only exactly a single output allowed.
  RET_CHECK(kOutFloat(cc).IsConnected() ^ kOutFloats(cc).IsConnected());
  return absl::OkStatus();
}

absl::Status TensorsToFloatsCalculatorNew::Open(CalculatorContext* cc) {
  options_ = cc->Options<::mediapipe::TensorsToFloatsCalculatorOptions>();
  return absl::OkStatus();
}

absl::Status TensorsToFloatsCalculatorNew::Process(CalculatorContext* cc) {
