...
#include "mediapipe/nui/desktop/calculators/resource_provider_calculator_new.h"
...
absl::Status ResourceProviderCalculatorNew::UpdateContract(CalculatorContext* cc) {
...
absl::Status ResourceProviderCalculatorNew::Open(CalculatorContext* cc) {
  const bool uses_side_packets = kIds(cc).Count() > 0;
  const auto& opts = cc->Options<ResourceProviderCalculatorOptions>();
...
MEDIAPIPE_REGISTER_NODE(ResourceProviderCalculatorNew)
