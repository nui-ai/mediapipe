...
absl::Status FaceToRegionCalculator::GetContract(
  ...
  cc->Inputs().Tag(kFacesTag).Set<std::vector<hand_tracking_mp_lean::Detection>>();
  cc->Outputs().Tag(kRegionsTag).Set<DetectionSet>();
  return absl::OkStatus();
}

absl::Status FaceToRegionCalculator::Open(hand_tracking_mp_lean::CalculatorContext* cc) {
...
absl::Status FaceToRegionCalculator::Process(hand_tracking_mp_lean::CalculatorContext* cc) {
  if (cc->Inputs().HasTag(kVideoTag) &&
  ...
  cv::Mat frame;
  if (cc->Inputs().HasTag(kVideoTag)) {
    frame = hand_tracking_mp_lean::formats::MatView(
  ...
  if (!cc->Inputs().Tag(kFacesTag).Value().IsEmpty()) {
    const auto& input_faces =
        cc->Inputs().Tag(kFacesTag).Get<std::vector<hand_tracking_mp_lean::Detection>>();
