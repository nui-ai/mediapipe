...
class ModelTaskGraph : public Subgraph {
 ...
 protected:
  ...
  //   - a tensor vector (std::vector<hand_tracking_mp_lean::Tensor>) input stream with tag
  //     "TENSORS", representing the input tensors to be consumed by the
  //     inference engine.
  //   - a tensor vector (std::vector<hand_tracking_mp_lean::Tensor>) output stream with tag
