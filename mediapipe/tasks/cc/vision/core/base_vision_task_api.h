...
class BaseVisionTaskApi : public tasks::core::BaseTaskApi {
      ...
      if (node.calculator() == "ImageToTensorCalculator") {
        ...
        hand_tracking_mp_lean::ImageToTensorCalculatorOptions options =
            node.options().GetExtension(
                hand_tracking_mp_lean::ImageToTensorCalculatorOptions::ext);
 ...
 protected:
  ...
  static absl::StatusOr<hand_tracking_mp_lean::NormalizedRect> ConvertToNormalizedRect(
      std::optional<ImageProcessingOptions> options,
      const hand_tracking_mp_lean::Image& image, bool roi_allowed = true) {
    hand_tracking_mp_lean::NormalizedRect normalized_rect;
