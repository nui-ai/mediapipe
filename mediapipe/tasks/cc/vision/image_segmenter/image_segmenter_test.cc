...
Image GetSRGBImage(const std::string& image_path) {
  ...
  hand_tracking_mp_lean::ImageFrame image_frame(
      hand_tracking_mp_lean::ImageFormat::SRGB, image_mat.cols, image_mat.rows,
      image_mat.step, image_mat.data, [image_mat](uint8_t[]) {});
  Image image(std::make_shared<hand_tracking_mp_lean::ImageFrame>(std::move(image_frame)));
  return image;
}
...
Image GetSRGBAImage(const std::string& image_path) {
  ...
  hand_tracking_mp_lean::ImageFrame image_frame(
      hand_tracking_mp_lean::ImageFormat::SRGBA, image_mat.cols, image_mat.rows,
      image_mat.step, image_mat.data, [image_mat](uint8_t[]) {});
  Image image(std::make_shared<hand_tracking_mp_lean::ImageFrame>(std::move(image_frame)));
  return image;
}
...
TEST_F(ImageModeTest, SucceedsWithConfidenceMask) {
  ...
  cv::Mat cat_mask = hand_tracking_mp_lean::formats::MatView(
...
TEST_F(ImageModeTest, SucceedsSelfie128x128Segmentation) {
  ...
  cv::Mat selfie_mask = hand_tracking_mp_lean::formats::MatView(
...
TEST_F(ImageModeTest, SucceedsHairSegmentation) {
  ...
  cv::Mat hair_mask = hand_tracking_mp_lean::formats::MatView(
      result.confidence_masks->at(1).GetImageFrameSharedPtr().get());
  MP_ASSERT_OK(segmenter->Close());
  cv::Mat expected_mask = cv::imread(
