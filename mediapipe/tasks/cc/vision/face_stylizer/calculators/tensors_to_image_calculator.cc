...
//
// Input streams:
//   TENSORS - std::vector<hand_tracking_mp_lean::Tensor> that only contains one element.
//
// Output streams:
//   OUTPUT - hand_tracking_mp_lean::Image.
...
#else
  GLuint program_ = 0;
  std::unique_ptr<hand_tracking_mp_lean::QuadRenderer> gl_renderer_;
#endif  // MEDIAPIPE_OPENGL_ES_VERSION >= MEDIAPIPE_OPENGL_ES_31
#endif  // MEDIAPIPE_METAL_ENABLED
#endif  // !MEDIAPIPE_DISABLE_GPU
};
MEDIAPIPE_REGISTER_NODE(::hand_tracking_mp_lean::tasks::TensorsToImageCalculator);
...
absl::Status TensorsToImageCalculator::CpuProcess(CalculatorContext* cc) {
  ...
  auto format = hand_tracking_mp_lean::ImageFormat::SRGB;
  ...
  cv::Mat output_matview = hand_tracking_mp_lean::formats::MatView(output_frame.get());
...
absl::Status TensorsToImageCalculator::MetalProcess(CalculatorContext* cc) {
  ...
  auto input_view =
      hand_tracking_mp_lean::MtlBufferView::GetReadView(input_tensor, command_buffer);
  [compute_encoder setBuffer:input_view.buffer() offset:0 atIndex:0];

  hand_tracking_mp_lean::GpuBuffer output =
...
absl::Status TensorsToImageCalculator::GlSetup(CalculatorContext* cc) {
  ...
  if (options.gpu_origin() != hand_tracking_mp_lean::GpuOrigin::TOP_LEFT) {
  ...
  const std::string src =
      absl::StrCat(hand_tracking_mp_lean::kMediaPipeFragmentShaderPreamble,
                   kFragColorOutputDeclaration, maybe_flip_y_define, kBody);
  gl_renderer_ = std::make_unique<hand_tracking_mp_lean::QuadRenderer>();
...
absl::Status TensorsToImageCalculator::GlProcess(CalculatorContext* cc) {
  ...
  return gl_helper_.RunInGlContext([this, cc]() -> absl::Status {
    ...
    auto texture_buffer = hand_tracking_mp_lean::GlTextureBuffer::Wrap(
        ...
        [ptr = out_texture.release()](
            std::shared_ptr<hand_tracking_mp_lean::GlSyncPoint> sync_token) mutable {
          delete ptr;
        });

    auto output =
        std::make_unique<hand_tracking_mp_lean::GpuBuffer>(std::move(texture_buffer));
    kOutputImage(cc).Send(Image(*output));
    ...
    MP_RETURN_IF_ERROR(gl_renderer_->GlRender(
        ...
        output_texture.height(), hand_tracking_mp_lean::FrameScaleMode::kStretch,
        hand_tracking_mp_lean::FrameRotation::kNone,
