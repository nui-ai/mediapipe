...
//   PLUGIN_TENSORS - std::vector<hand_tracking_mp_lean::Tensor> @Optional
...
//
// Outputs:
//   IMAGE - hand_tracking_mp_lean::ImageFrame
...
class StableDiffusionIterateCalculator : public Node {
 public:
  ...
  static constexpr Output<hand_tracking_mp_lean::ImageFrame> kImageOut{"IMAGE"};
...
absl::Status StableDiffusionIterateCalculator::Open(CalculatorContext* cc) {
  ...
  MP_RETURN_IF_ERROR(hand_tracking_mp_lean::file::Exists(config.model_dir))
      << config.model_dir;
...
absl::Status StableDiffusionIterateCalculator::Process(CalculatorContext* cc) {
  ...
  if (kIterationIn(cc).IsEmpty()) {
    ...
    if (show_result) {
      ...
      kImageOut(cc).Send(Packet<hand_tracking_mp_lean::ImageFrame>());
    }
  }
  return absl::OkStatus();
}
