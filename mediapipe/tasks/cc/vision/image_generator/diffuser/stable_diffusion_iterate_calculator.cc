...
//   PLUGIN_TENSORS - std::vector<mediapipe_v01013_based::Tensor> @Optional
...
//
// Outputs:
//   IMAGE - mediapipe_v01013_based::ImageFrame
...
class StableDiffusionIterateCalculator : public Node {
 public:
  ...
  static constexpr Output<mediapipe_v01013_based::ImageFrame> kImageOut{"IMAGE"};
...
absl::Status StableDiffusionIterateCalculator::Open(CalculatorContext* cc) {
  ...
  MP_RETURN_IF_ERROR(mediapipe_v01013_based::file::Exists(config.model_dir))
      << config.model_dir;
...
absl::Status StableDiffusionIterateCalculator::Process(CalculatorContext* cc) {
  ...
  if (kIterationIn(cc).IsEmpty()) {
    ...
    if (show_result) {
      ...
      kImageOut(cc).Send(Packet<mediapipe_v01013_based::ImageFrame>());
    }
  }
  return absl::OkStatus();
}
