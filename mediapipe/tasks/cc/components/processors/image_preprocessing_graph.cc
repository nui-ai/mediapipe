...
using ::hand_tracking_mp_lean::tasks::vision::ImageTensorSpecs;
...
absl::Status ConfigureImageToTensorCalculator(
    ...
    hand_tracking_mp_lean::ImageToTensorCalculatorOptions* options) {
...
class ImagePreprocessingGraph : public Subgraph {
 ...
 private:
  ...
  // (hand_tracking_mp_lean::Image) and region of interest (hand_tracking_mp_lean::NormalizedRect) as
  // inputs and returns 5 output streams:
  //   - the converted tensor (hand_tracking_mp_lean::Tensor),
  ...
  // image_in: (hand_tracking_mp_lean::Image) stream to preprocess.
...
REGISTER_MEDIAPIPE_GRAPH(
    ::hand_tracking_mp_lean::tasks::components::processors::ImagePreprocessingGraph)
