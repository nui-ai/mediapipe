...
absl::Status ConvertHfTokenizer(const std::string& hf_tokenizer,
  ...
  absl::string_view output_dir = ::hand_tracking_mp_lean::file::Dirname(output_vocab_path);
  if (!::hand_tracking_mp_lean::file::IsDirectory(output_dir).ok()) {
    MP_RETURN_IF_ERROR(::hand_tracking_mp_lean::file::RecursivelyCreateDir(output_dir));
  }

  MP_RETURN_IF_ERROR(hand_tracking_mp_lean::file::SetContents(
